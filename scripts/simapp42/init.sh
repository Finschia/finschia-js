...
timeout 60 bash -c "until curl -s http://localhost:1318/node_info > /dev/null; do sleep 0.5; done"
...
timeout 20 bash -c "until [ \"\$( curl -s http://localhost:1318/blocks/latest | jq -r '.block.header.height // 0' )\" -ge 1 ]; do sleep 0.5; done"
